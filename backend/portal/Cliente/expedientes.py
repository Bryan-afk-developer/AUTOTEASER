...
def calcular_estados_de_cuenta(banco: dict = None, docs_subidos: dict = None) -> list[dict]:
    ...
    slots_to_generate = max_uploaded + 1
...
async def get_expediente(authorization: str = Header(None)):
    ...
    for doc_subido in todos_subidos:
        ...
        if tipo.startswith("declaracion_"):
            continue
        if tipo not in req_claves and (tipo.startswith("otros_") or tipo.startswith("financiero_eeff_")):
            estado = doc_subido["estado"]
            url_documento = None
            storage_path = doc_subido.get("storage_path")
            if storage_path:
...
async def procesar_acta_principal(clave: str, authorization: str = Header(None)):
    ...
    # Analizar con Gemini o extraer texto
    from app.acta_processor import analyze_acta
    try:
        ai_data = analyze_acta(pdf_bytes)
    except Exception as e:
        logger.error(f"Error procesando acta: {e}")
